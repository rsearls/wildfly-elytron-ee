...
**/
//rls import org.picketlink.identity.federation.core.saml.v2.util.DocumentUtil;
...
public abstract class AbstractPicketLinkTrustHandler<C extends LogicalMessageContext> implements SOAPHandler {
    private static final Logger logger = Logger.getLogger(AbstractPicketLinkTrustHandler.class.getName());
    // rls protected static final PicketLinkLogger logger = PicketLinkLoggerFactory.getLogger();

    protected static Set<QName> headers;

    private static String JAAS_CONTEXT_ROOT = "java:jboss/jaas/";  // rls
    protected static final String SEC_MGR_LOOKUP = JAAS_CONTEXT_ROOT; // rls SecurityConstants.JAAS_CONTEXT_ROOT;
     ...
     * @return
     *
     * @throws GeneralSecurityException if no security domain is configured.
     */
    protected String getSecurityDomainName(MessageContext msgContext) throws GeneralSecurityException {
        ...
        if (this.securityDomainName == null) {
            // rls LOGGER.log(Level.INFO, "Security domain not found");
            throw new GeneralSecurityException("The security domain name could not be found. Check your jboss-web.xml.");
        }
    ...
    private InputStream getJBossWeb(ServletContext context) {
        if (context == null) {
            throw new RuntimeException(ErrorCodes.NULL_VALUE + "Servlet Context");
            //rls throw logger.nullValueError("Servlet Context");
        }
    ...
    protected void trace(MessageContext msgContext) {
        if (msgContext instanceof SOAPMessageContext) {
            ...
            logger.log(Level.INFO, "WSDL_PORT=" + soapMessageContext.get(SOAPMessageContext.WSDL_PORT));
            logger.log(Level.INFO, "WSDL_OPERATION=" + soapMessageContext.get(SOAPMessageContext.WSDL_OPERATION));
            logger.log(Level.INFO, "WSDL_INTERFACE=" + soapMessageContext.get(SOAPMessageContext.WSDL_INTERFACE));
            logger.log(Level.INFO, "WSDL_SERVICE=" + soapMessageContext.get(SOAPMessageContext.WSDL_SERVICE));
        }
    }
    ...
    protected Element getAssertionFromSubject() {
        ...
        if (subject == null) {
            logger.log(Level.FINE, "No authentication Subject found, cannot provide any user roles!");
            return assertion;
        }
            ...
            for (Object cred : creds) {
                if (cred instanceof SamlCredential) {
                    ...
                    try {
                        ...
                        // rls logger.samlAssertionPasingFailed(e);
                        logger.log(Level.SEVERE, "SAML Assertion parsing failed", e);
                    }
                    break;
                }
            }
